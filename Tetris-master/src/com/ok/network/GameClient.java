...
public class GameClient implements Runnable{
	...
	public void run(){
		...
		while(true){
			...
			}else if(data.getCommand() == DataShip.SEND_MAP) {
				if (data.isserver != tetris.isserver)
				{
					System.out.println("Comp");
					tetris.setmap(data.temp);
				}			
			}else if(data.getCommand() == DataShip.MAP_BLIND) {
				if (isPlay)
					reBlind(data.getMsg(), data.getIndex());
			}
	...
	public void SendMap(Block[] map, int isserver) {
		Block[] temp = map;
		//for (int i = 0; i < 210; i++)
		//{
		//	if (temp[i] != null)
		...
		//}
		DataShip data = new DataShip(DataShip.SEND_MAP);
		data.temp = temp;
		//for (int i = 0; i < 210; i++)
		//		{
		//			if (data.temp[i] != null)
		//				System.out.println(data.temp[i].getX());
		//		}
	...
	public void Blind()
	{
		DataShip data = new DataShip(DataShip.MAP_BLIND);
		send(data);
	}
	
	public void reBlind(String msg, int index)
	{
		if (index != this.index) tetris.getBoard().BlindAction();
		rePrintSystemMessage(msg);
	}
}
