...
//TODO:--------------------------[ 요청 대기 ]-------------------------
	...
	public void map(Block[] map, int isserver) {
		if (list.size() == 2)
		{
			DataShip data2 = new DataShip(DataShip.SEND_MAP);
			data2.temp = map;
		//for (int i = 0; i < 210; i++)
		//{
		//	if (data2.temp[i] != null)
		//		System.out.println(data2.temp[i].getX());
		//}
			data2.isserver = isserver;
			broadcast(data2);
		}
	}
	...
	public void blind() {
		...
		data.setMsg(index+"P -> Activate Blind");
		data.setIndex(index);
		broadcast(data);
	}
